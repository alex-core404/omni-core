...
def postprocess_ai_response(text: str) -> str:
    ...
    if "PS Саша ->❤️" not in text:
        ...
        if not text.endswith(('.', '!', '?')):
            text += '.'
        text += '\nPS Саша ->❤️ Ты замечательная.'
    text = re.sub(r'(PS Саша ->❤️.*?)\b(жду|скучаю)\b', r'\1', text, flags=re.IGNORECASE)
        ...
        while True:
            ...
            if is_ai:
                ...
                if is_personal_ai:
                    ...
                    4. Фраза в PS должна быть позитивной, без слов "жду", "скучаю". Используй комплименты, поддержку, нежность. Примеры: "PS Саша ->❤️ Ты у меня самая лучшая", "PS Саша ->❤️ Твоя улыбка делает мой день", "PS Саша ->❤️ Ты замечательная".
