        ...
        while True:
            ...
            if is_ai:
                ...
                if is_romantic and user_email in SPECIAL_PAIR and message_data["to"] in SPECIAL_PAIR:
                    system = "Ты помощник в мессенджере Omni. Отвечай кратко и по делу. Не используй markdown. Отвечай только на русском языке. После ответа добавь одну короткую фразу от Александра его девушке — пожелание хорошего дня или настроения плюс простой комплимент. Естественно, без пафоса. Каждый раз что-то новое. Формат: PS от Александра: [фраза]"
                else:
